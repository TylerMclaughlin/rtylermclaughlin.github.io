...
The recent availability of data this large has motivated the development of new methods for its analysis. 
...
This initial model has a multiple R-squared of  `summary(lm1)$r.squared`.  This isn't bad for what I expected.  
...
```
 ...
 Well, not quite.  We also need to compare the RMSE values to the statistics of the response variable, CD56.
Otherwise we shouldn't make hay out of the model prediction.
...
With RMSE of about `r ridge.RMSE`,it looks like our model makes predictions within one standard deviation of the sample mean.  This is pretty OK!
...
These plots indiate there are no dramatic non-linearities present in the data.  This suggests that using linear models seems to be an ok way to make predictions and that there is no need to use more advanced tools like polynomial regression or splines for a task like this. 
It is worth mentioning that **the logarithmic transform** during the pre-preprocessing stage **was essential** as otherwise these *residuals vs fitted values* plots would look much less well-behaved.
...
Ideally, I would like to have prediction accuracy within a small fraction of the sample standard deviation.  The high RMSE relative to scale of CD56 variablitiy means that measuring the expression level of these 34 proteins per cell is not sufficient to infer the expression level of CD56. 
